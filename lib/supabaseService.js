...
// ─── Record Transaction ───────────────────────────────────────
/**
 * Records an expense transaction and logs it in whatsapp_transactions.
 * This is wrapped in a logical sequence (not a DB transaction, but Supabase
 * triggers handle the account balance update automatically).
 *
...
export async function recordExpenseTransaction(receiptData, accountId, senderJid) {
  ...
  const { data: txData, error: txError } = await supabase
    ...
    .insert([{
      ...
      type:             'expense',         // always expense (outgoing)
      amount:           amount,
  ...
  logger.success(`Transaction recorded! ID: ${txData.id}`);
  ...
  const { error: logError } = await supabase
    ...
    .insert([{
      ...
      provider:            provider,
      raw_extracted_data:  receiptData,
      sender_jid:          senderJid,
      status:              'processed',
    }]);

  if (logError) {
    // Non-fatal: transaction already saved, just log the warning
