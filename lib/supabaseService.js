...
export async function findAccountByWalletNumber(walletNumber) {
  ...
  // Convert Arabic digits and remove spaces/dashes
  let normalized = walletNumber
  ...
  // 1. Try exact match with 11-digit local format (01xxxxxxxxx)
  ...
  if (!data && normalized.startsWith('01') && normalized.length === 11) {
    // 2. Try match with country code prefix (+201... or 201...) if DB stores international format
    const withPlus = '+20' + normalized.slice(1);
    const withCode = '20' + normalized.slice(1);

    const { data: altData } = await supabase
      .from('accounts')
      .select('id, account_number, owner_name, current_balance')
      .or(`account_number.eq.${withPlus},account_number.eq.${withCode}`)
      .maybeSingle();
